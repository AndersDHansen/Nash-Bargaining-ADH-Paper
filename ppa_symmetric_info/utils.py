...
def calculate_cvar_left(earnings: np.ndarray, probabilities: pd.DataFrame, alpha: float) -> float:
    """Calculate the left-tail Conditional Value at Risk (CVaR).

    Computes the conditional expectation of earnings in the worst-case
    scenarios whose cumulative probability mass equals (1 - alpha).
    For example, alpha = 0.95 returns the expected earnings in the
    worst 5% of outcomes.

    Parameters
    ----------
    earnings : ndarray
        Array of earnings values per scenario
    probabilities : pd.DataFrame or array-like
        Probability weight for each scenario (must sum to 1)
    alpha : float
        Confidence level (e.g., 0.95). The left tail covers
        cumulative probability mass = 1 - alpha.

    Returns
    -------
    float
        Conditional expectation of earnings in the left tail
    """
...
def _left_tail_mask(arr, probabilities, alpha):
    """
    Identify the left-tail boundary in probability-sorted order.

    Sorts scenarios from worst to best, accumulates probability mass,
    and finds the boundary index where cumulative mass reaches (1 - alpha).

    Args:
        arr: array of values (e.g., earnings per scenario)
        probabilities: array of probabilities for each scenario
        alpha: confidence level (e.g., 0.95 means worst 5% tail)

    Returns:
        order: index array sorting scenarios from worst to best
        boundary_idx: index in sorted order where the tail boundary falls
    """
    tail_prob = 1.0 - alpha
    ...
    return order, boundary_idx

def _left_tail_weighted_sum(probabilities, values,
                             order, boundary_idx, tail_prob):
    """
    Probability-weighted conditional expectation over the left tail,
    taking only the fractional weight of the boundary scenario.

    Parameters:
    -----------
    probabilities : array-like
        Scenario probabilities (original order)
    values : array-like
        Per-scenario values (original order)
    order : array
        Indices sorting scenarios from worst to best
    boundary_idx : int
        Index in sorted order where the tail boundary falls
    tail_prob : float
        Confidence level (e.g., 0.95); tail mass = 1 - tail_prob

    Returns:
    --------
    float : Conditional expectation over the left tail
    """
...
def _calculate_S_star_PAP_G(x,gamma,A,alpha, production,price,capture_rate,PROB):
    """
    Calculate S* for the generator in PAP (pay-as-produced) contracts.

    S* is the strike price boundary derived from the individual rationality
    constraint: the price at which the generator is indifferent between
    entering and not entering the contract.
    """
    S =x[0]
...
def _calculate_S_star_PAP_L(x,gamma,A,alpha, production,price,capture_rate,load_CR, load_scenarios, PROB):
    """
    Calculate S* for the load in PAP (pay-as-produced) contracts.

    S* is the strike price boundary derived from the individual rationality
    constraint: the price at which the load is indifferent between
    entering and not entering the contract.
    """
    S = x[0]
...
def _calculate_S_star_BL_G(x, M, A, alpha, production, price, capture_rate, PROB, direction, discount_rate=None, n_time=None):
    """Calculate S* for the generator in baseload contracts.

    S* is the strike price boundary from the individual rationality constraint:
    the price at which the generator is indifferent between entering and not
    entering the contract. Direction controls whether this is the upper (S^U)
    or reservation (S^R) boundary.
    """
    S = x[0]
...
def _calculate_S_star_BL_L(x, M, A, alpha, price, load_CR, load_scenarios, PROB, direction, discount_rate=None, n_time=None):
    """Calculate S* for the load in baseload contracts.

    S* is the strike price boundary from the individual rationality constraint:
    the price at which the load is indifferent between entering and not
    entering the contract. Direction controls whether this is the upper (S^U)
    or reservation (S^R) boundary.
    """
    S = x[0]
...
def optimize_nash_product(
    ...
    def calculate_utilities(S: float, M: float) -> tuple[float, float, float]:
        ...
        # Use provided probabilities or fall back to uniform
        prob = probabilities if probabilities is not None else np.ones(len(net_earnings_no_contract_G)) / len(net_earnings_no_contract_G)
        ...
        earnings_G = Expected + rev_contract_total
        CVaR_G = calculate_cvar_left(earnings_G, prob, alpha)

        UG = (1-A_G)*(prob * earnings_G).sum() + A_G * CVaR_G

        rev_contract = M  * ( price_L - S )
        rev_contract_total = rev_contract.sum(axis=0)
        Expected = net_earnings_no_contract_L
        earnings_L = Expected + rev_contract_total
        CVaR_L = calculate_cvar_left(earnings_L, prob, alpha)
        
       
        UL = (1-A_L)*(prob * earnings_L).sum() + A_L * CVaR_L
