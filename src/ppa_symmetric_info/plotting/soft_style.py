...
def _build_cmaps():
    ...
    sequential = LinearSegmentedColormap.from_list(
        "soft_sequential",
        ["#F4845F", "#E05C3A", "#CC2929", "#8B1515"],
        N=256,
    )
    sequential_blue = LinearSegmentedColormap.from_list(
        "soft_sequential_blue",
        ["#BCD8D6", "#28AAAA", "#2878CC", "#1A5B81"],
        N=256,
    )
    ...
    return sequential, sequential_blue, diverging


CMAP_SEQUENTIAL, CMAP_SEQUENTIAL_BLUE, CMAP_DIVERGING = _build_cmaps()
...
def figure_title(fig, text: str, y: float | None = None) -> None:
    ...
    """
    kwargs = dict(fontsize=18, fontweight="bold", color=NEUTRAL["title"], ha="center")
    if y is not None:
        kwargs["y"] = y
...
def panel_subtitle(ax, text: str) -> None:
    ...
    ax.set_title(text, color=NEUTRAL["label"], fontsize=12, fontweight="normal", pad=10)


def better_badge(ax, direction: str = "down", loc: str = "upper right") -> None:
...
def save_figure(fig, name: str, formats=("png",), dpi: int = 300):
...
def _rounded_bar_path(
    x_center: float, height: float, width: float, r_x: float, r_y: float
):
...
def rounded_bars(
    ...
    **Set ax.set_ylim() and ax.set_xlim() BEFORE calling rounded_bars()
    ...
    for i, (xi, h) in enumerate(zip(xs, hs)):
        path = _rounded_bar_path(float(xi), float(h), width, r_x_data, r_y_data)
        if path is None:
            continue
        lbl = label if (i == 0 and label is not None) else None
        patch = PathPatch(
            path, facecolor=color, edgecolor="none", label=lbl, zorder=zorder, **kwargs
        )
        ax.add_patch(patch)
...
def _rounded_hbar_path(
    y_center: float, width: float, height: float, r_x: float, r_y: float
):
...
def rounded_hbars(
    ...
    **kwargs,
    ...
    for i, (yi, w) in enumerate(zip(ys, ws)):
        path = _rounded_hbar_path(
            float(yi), float(w), float(height), r_x_data, r_y_data
        )
        if path is None:
            continue
        lbl = label if (i == 0 and label is not None) else None
