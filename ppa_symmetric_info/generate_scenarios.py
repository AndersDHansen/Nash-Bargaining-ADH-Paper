...
def _save_matrix(folder: Path, kind: str, mat: np.ndarray, start: str | pd.Timestamp, resample: bool, monte_price: bool = False):
...
def run_scenarios(
    *,
    ...
    _save_matrix(out, "price", price_mat, start_time, resample=True, monte_price=monte_price)

    # 2) production
    prod_mdl = ProductionModel.from_csv(prod_csv_path, capacity_mw, seed)
    prod_mat = prod_mdl.simulate(years, num_scenarios)
    _save_matrix(out, "production", prod_mat, start_time, resample=True, monte_price=monte_price)
    ...
    _save_matrix(out, "capture_rate", cr_mat, start_time, resample=False, monte_price=monte_price)

    # 4) load
    load_mdl = LoadModel.from_csv(consumption_csv_path, seed)
    load_mat = load_mdl.simulate(years, num_scenarios)
    _save_matrix(out, "load", load_mat, start_time, resample=True, monte_price=monte_price)
    # 5) load capture rate
    ...
    _save_matrix(out, "load_capture_rate", load_cr_mat, start_time, resample=False, monte_price=monte_price)

    print(f"Wrote scenarios to {out.resolve()}")

if __name__ == "__main__":
    from config_scenarios import (
        YEARS, NUM_SCENARIOS, MONTE_PRICE, START_TIME,
        WIND_CSV, PRICE_CSV, CONSUMPTION_CSV,
        CAPACITY_MW, OUTPUT_DIR,
    )
    ...
    prod_csv_path = WIND_CSV
    consumption_csv_path = CONSUMPTION_CSV
