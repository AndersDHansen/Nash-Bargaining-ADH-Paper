...
from config_optimization import (
    ...
    NUM_SENSITIVITY, A_G_VALUES, A_L_VALUES, TAU_L_VALUES, TAU_G_VALUES,
)
import copy
import json
import timeit
...
def run_contract_negotiation(input_data: InputData): 
        ...
        new_base = copy.deepcopy(base_input)
...
def save_results_to_csv(results_dict, contract_type, time_horizon, num_scenarios,
                        monte_price=False, A_G=0.5, A_L=0.5):
...
def main():
    # All parameters loaded from config_optimization.py
    A_L = _A_L
    A_G = _A_G
    scenario_time_horizon = SCENARIO_TIME_HORIZON
    ...
    selected_analyses = list(SELECTED_ANALYSES)

    num_sensitivity = NUM_SENSITIVITY
    ...
    if sensitivity == True:
        ...
        save_results_to_csv(results, contract_type, opt_time_horizon, num_scenarios,
                            monte_price=monte_price, A_G=A_G, A_L=A_L)
    else:
